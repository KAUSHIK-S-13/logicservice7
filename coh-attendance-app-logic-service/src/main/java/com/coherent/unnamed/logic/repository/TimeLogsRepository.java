...
public interface TimeLogsRepository  extends JpaRepository<TimeLogs, Long> {
    ...
    @Query(value = "select * from time_logs gets where date(gets.created_at) = :strDate and gets.user_id_fk = :id and gets.is_logged=:isLogged and gets.status=true", nativeQuery = true)
