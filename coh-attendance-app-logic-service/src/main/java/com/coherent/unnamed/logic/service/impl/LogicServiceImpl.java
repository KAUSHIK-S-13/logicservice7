		...
		originalId.stream().forEach(id -> {
			...
			timeLogsPunchIn.stream().forEach(timeLogs1 -> {
				timeLogsPunchOut.stream().forEach(timeLogs2 -> {
					int loginDate = Integer.parseInt(null);
					int logoutDate = Integer.parseInt(null);
					loginDate = timeLogs1.getCreatedAt().toLocalDateTime().toLocalTime().getHour();
					logoutDate = timeLogs2.getCreatedAt().toLocalDateTime().toLocalTime().getHour();
					long diff = logoutDate- loginDate;
					value[0] =diff;
				});
			});
